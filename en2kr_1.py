...
# __init__
learning_rate = 1e-7
n_input = 26 + 3  # 29 = Alphabet + S/E/P
n_hidden = 256
n_class = 19 + 21 + 1 + 3  # 44 = 초성 + 중성 + 종성없을때 + S/E/P
total_epoch = 14000
t_epoch = 100
word_limit = 10
...
char_arr = [c for c in 'SEPabcdefghijklmnopqrstuvwxyzㄱㄴㄷㄹㅁㅂㅅㅇㅈㅊㅋㅌㅍㅎㅏㅐㅑㅓㅔㅕㅗㅘㅚㅛㅜㅝㅟㅠㅡㅣ']
...
def make_batch(seq_data, limit):
    ...
    for seq in seq_data:
        ...
        input = pad(input, limit)
        # 디코더 셀의 입력값. 시작을 나타내는 S 심볼을 맨 앞에 붙여준다.
        output = [num_kr[n] for n in ('S' + seq[1])]
        output = pad(output, limit*1.5)
        # 학습을 위해 비교할 디코더 셀의 출력값. 끝나는 것을 알려주기 위해 마지막에 E 를 붙인다.
        target = [num_kr[n] for n in (seq[1] + 'E')]
        target = pad(target, limit*1.5)

        input_batch.append(np.eye(n_input)[input])
...
enc_input = tf.placeholder(tf.float32, [None, None, n_input], name='enc_input')  # [batch size, time steps, input size]
dec_input = tf.placeholder(tf.float32, [None, None, n_class], name='dec_input')  # [batch size, time steps, class size]
targets = tf.placeholder(tf.float32, [None, None, n_class], name='targets')  # [batch size, time steps, class size]
...
test_cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=model, labels=targets, name='test_softmax'))
...
for epoch in range(start_step, total_epoch+1):
    ...
    if epoch % t_epoch == 0:
        ...
        loss = sess.run(test_cost, feed_dict={enc_input: test_input,
...
def predict(word):
    ...
    print(result[0])
    decoded = [kr_arr[i] for i in result[0]]
...
print('abbyville ->', predict('abbyville'))
